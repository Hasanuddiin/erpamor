				...
				<table  class="table table-bordered table-hover dataTable" id="example1">
					<thead>
						...
						<tbody>
							...
							<tr>
								<td><?= $i ?></td>
								<td class="nr"><?= $data->invoiceBuyID ?></td>
								<td><?= $data->invoiceOrderID ?></td>
								...
								<td>
									<button class="btnShow1 btn btn-info btn-sm" id="btnShow1"  value="<?= $data->invoiceBuyID ?>" ><i class="fa fa-search"></i> Detail</button>
								</td>
		...
		<script>
			...
			$(document).ready(function(){
				...
				$.ajax({
					...
					url:'<?php echo base_url("penerimaan/pop_detail/"); ?>',
					data:{'id':id},
