        ...
        if($r->productDiscounttype=="1")
        {
         ...
         <td>
           <div class="btn-group" role="group">
            <a href="#" class="btnShow btn btn-info btn-sm" id="btnShow" ideditpro="<?php echo $r->detailID ?>"nampro="<?php echo $r->productName; ?>" harpro="<?php echo $r->detailBuyPrice; ?>" qtypro="<?php echo $r->detailBuyQty; ?>" nofakedit="<?php echo $trxid;?>" productbarcode="<?php echo $r->productBarcode ?>"><i class="fa fa-edit"></i> Edit</a>
            <input type="hidden" name="qtyremove" id="qtyremove">
            <a href="<?php echo base_url(); ?>pembelian/hapus_tpm?idtrxpj=<?php echo $r->detailID ?>&productBarcode=<?php echo $r->productBarcode; ?>&qtyremove=<?php echo $r->detailBuyQty ?>&nofak=<?php echo $trxid;?>&tgltrx=<?php echo $trxDate; ?>" onclick="javascript: return confirm('Anda akan menghapus item : <?php echo $r->productName; ?> ?')" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i> Hapus</a>
          </div>
        </td>
      </tr>
      <?php 
...
<div class="col-md-6">
 <div class="form-group">
Dikirim Ke :
  <input type="hidden"  class="form-control" name="namamember3"  />
  ...
  <select name="member" id="member" class="form-control">
    ...
    foreach ($member as $member):
     ...
     ?>
   <option value="<?= $member->memberID ?>"  SELECTED ><?php echo $member->memberFullName ?></option>
...
<div class="col-md-12">
  ...
  <div class="col-md-4" id="termindate">
    <div class="form-group has-warning">
     Jatuh Tempo (Hari):
     <input type="text"  name="terminpjdate" class="form-control">
   </div>
 </div>
</div>
</div>
...
$(document).ready(function(){
  ...
  $( "#suplier" ).on('change', function() {
    ...
    $.ajax({
      ...
      $("input[name='telpsuplier']").val(duce.supplierPhone);
   ...
   $( "#member" ).on('change', function() {
    ...
    $.ajax({
      type:'POST',
      // dataType: 'jsonp',
      url:'<?php echo base_url("welcome/members/"); ?>',
      data:{'id':id},
      success:function(data){
        // $('#resultdiv').html(data);
        var duce = jQuery.parseJSON(data);
      $("input[name='alamatkirim']").val(duce.memberAddress);
      $("input[name='telpkirim']").val(duce.memberPhone);
      $("input[name='namamember3']").val(duce.memberFullName);
      $("input[name='memberID']").val(duce.memberID);
