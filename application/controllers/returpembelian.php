...
class Returpembelian extends CI_Controller
{
	...
	function addtrt() {
		...
		$a['datapo']	= $this->model_master->tampil_po()->result_object();
		if($this->input->post('submit') == "tambah")
		{
			...
			else if($this->input->post('submit') == 'selesaitrx')
			{
				...
				$data = $this->model_master->tampil_debt_id($invoiceOrderID);
				...
				foreach($data->result() as $row)
			{
				...
				$debID = $row->debtID;
					...
					$object2 = array(
						'debtPay' => $trxTotalnum,
						'debtID' => $debID,
						'keterangan' => 'Retur Pembelian dengan no PO : '.$invoiceOrderID.'' ,
						'createdDate' => $trxDate
						
						);
					$this->db->insert('as_debts_payment', $object2);
				

				$a['perusahaan']= $this->model_perusahaan->tampil_data()->result();
				$a['membersupplier']= $this->model_master->tampil_suplier_data($supplierID)->result();
				$a['nofak']= $invoiceID;
		...
		function showtrxrt()
		{
		$a['data'] = $this->model_master->tampil_trxrtpem()->result_object();
		$a['page'] = "master/retur/show-trxrt";
