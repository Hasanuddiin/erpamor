...
public function hapus_trxbahan($id)
	...
	public function tampil_stok_produk($today)
	{
		$this->db->select('*');
		$this->db->from('as_products');
		$this->db->where('identityID',5);
		$this->db->where('productStock != 0');
		return $this->db->get();
	}

	public function tampil_stok_produkCicurug($today)
	{
		$this->db->select('*');
		$this->db->from('as_products');
	...
	public function tampil_rekaptodayToko($today,$user)
	{
	
		$this->db->select('as_products.productName as produk');
		$this->db->select('Sum(as_sales_transactions_toko.trxSubtotal) AS Rp');
		$this->db->select('Sum(as_sales_detail_transactions.detailQty) AS jumlah');
		$this->db->select('as_products.productSalePrice as harga');
    ...
    public function tampil_trxsumtodaytoko($today)
	{
	 ...
	 public function tampil_trxsumtodaytokoCicurug($today)
	{
		$this->db->select('SUM(trxTotal)As Totaltoday');
		$this->db->from('as_sales_transactions_toko');
		$this->db->where('trxDate', $today);
	...
	public function tampil_trxpjtodayToko($today)
	{
		...
		$this->db->where('trxDate', $today);
		$this->db->where('as_sales_transactions_toko.identityID',5);
		$this->db->where('as_pegawai.nip = as_sales_transactions_toko.userID');

		return $this->db->get();
	}
	
	public function tampil_trxpjtodayTokoCicurug($today)
	{
		$this->db->select('*');
		$this->db->from('as_sales_transactions_toko');
		$this->db->from('as_pegawai');
	...
	public function tampil_trxtotalCicurug($bulan)
	{
		$this->db->select('SUM(trxTotal)As total_bulan');
		$this->db->from('as_sales_transactions_toko');
		$this->db->where('identityID',6);
	...
	public function tampil_trxtodayCicurug($today)
	{
		$this->db->select('SUM(trxTotal)As Totaltoday');
		$this->db->from('as_sales_transactions_toko');
		$this->db->where('trxDate', $today);
...
//Purchasing Order
    ...
    function tampilkan_transaksi_all_order_by_tgl($start,$end)
    {	
